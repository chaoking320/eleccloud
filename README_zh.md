...
<p align="center">
  <img src="docs/images/origin_story.gif" alt="ElecCloud 量子物理到分布式自愈防御对比" width="100%" />
</p>

<p align="center">
  <a href="docs/simulation.html">
    <img src="https://img.shields.io/badge/🎮_在线交互演示-量子%E2%9F%B7分布式自愈模拟器-blueviolet?style=for-the-badge" alt="在线演示" />
  </a>
</p>

> **为什么叫 ElecCloud（电子云）？**
>
> 在量子力学中，单个电子的瞬时运动是不可预测的（测不准原理，如同分布式网络中突发的抖动与下游超时）；但在宏观上，无数电子在核外形成的**概率云**，却构成了原子最稳固的物理边界，使原子核免受外界宇宙射线的轰击。
>
> **ElecCloud 就是你核心业务系统的“电子云防御层”**：微观异常不可避免，宏观业务坚如磐石！

### 🛡️ 核心作用直观对照 (Why ElecCloud?)

| 生产高危痛点场景 | ❌ 传统裸奔模式 (无重试 / 简单 for 循环) | 🛡️ ElecCloud 电子云防御模式 |
| :--- | :--- | :--- |
| **突发 504 / 网络抖动** | 瞬间耗尽 Tomcat 业务线程池，引发全站雪崩，订单直接流失 | **非阻塞脱敏**：200ms 本地快重试，超限平滑升级为 Redis 延时削峰，主线程阻塞 0ms |
| **下游接口 429 频繁限流** | 立即反复重试加剧下游封禁，引发恶性风暴 | **指数退避抖动 (Jitter)**：打散重试流量平滑削峰，等待下游恢复后最终自愈 |
| **集群多节点并发冲突** | 多个微服务实例同时重试同一笔订单导致超卖或重复结算 | **ShedLock 互斥环**：集群锁保证全局单次幂等安全执行 |
| **接入成本与代码侵入** | 每个业务接口写大量繁琐的 try-catch、MQ 补偿与定时任务 | **一行注解零侵入**：`@RetryableTask(sceneType = 1001)` 全自动就位 |
...
- **对比 Spring Retry**：Spring Retry 是优秀的单机内存重试工具，但进程退出即丢失，且长时间阻塞会打满线程池。ElecCloud 将两者融会贯通，推出**两级协同重试机制**：针对网络瞬断支持毫秒级本地快重试，用尽后自动平滑升级为分布式持久化延时重试。
...
| **两级重试协同** | ❌ 仅本地内存重试 | ❌ 仅慢周期定时 | ❌ 仅远程重试 | ✅ **原生两级协同（快重试+慢兜底）** |
...
### 2. 两级协同重试架构（本地快重试 + 分布式慢兜底）
针对 50ms 网络闪断、连接瞬时重置等瞬时微抖动，无需立刻入库或发送延时消息产生写放大。通过配置 `localRetryTimes` 与 `localIntervalMs`，SDK 会优先在当前线程本地快速重试：
- **瞬时自愈**：本地重试成功直接返回业务结果，**零 DB、零 Redis、零 MQ 开销**；
- **持续故障平滑升级**：本地重试用尽后，**自动平滑升级至分布式重试**（MySQL 入库 + MQ 延时调度 + 指数退避），并释放当前工作线程，防止线程池打满。

```java
@RetryableTask(
    sceneType = 1001,
    idempotentKey = "#orderId",
    localRetryTimes = 2,      // 第 1 级：本地毫秒级快重试 2 次
    localIntervalMs = 200     // 间隔 200ms（避免写放大与不必要的延迟）
    // 第 2 级：本地耗尽后自动平滑升级为分布式持久化慢重试
)
